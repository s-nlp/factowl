...
class NPM(LM):
    ...
    def tokenize(self, texts, skip_special_tokens=False, padding=True):
        ...
        all_input_ids = self.tokenizer(texts, truncation=True)["input_ids"]
    ...
    def encode(self, texts, skip_special_tokens=False, gt_input_ids=None):
        ...
        results = []
        for i, (text, input_ids, logits, hidden_states) in enumerate(
                zip(texts, all_input_ids, all_logits, all_hidden_states)):
    ...
    def get_probabilty(self, topic, question, k):
        passages = self.bm25.get_passages(topic, question, k=k)
        ...
        if cache_key not in self.cache_dict:
            ...
            all_scores = np.exp(np.inner(stacked_question_vectors, stacked_passage_vectors) / np.sqrt(
                stacked_passage_vectors.shape[-1]))
