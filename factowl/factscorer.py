...
from factowl.atomic_facts import AtomicFactGenerator
from factowl.clm_vllm import FactVerificator
...
class FactowlFactScorer(object):
    def __init__(self,
                 vllm_model,
        ...
        self.vllm_verifier = FactVerificator(vllm_model, model_name=self.llm_dir_or_name, debug=debug,
        ...
        if self.precomputed_passages is not None:
            self.extr_ps = '-extra'
        self.filter_facts = filter_facts
    ...
    def _get_score_vllm_batched(self, batch_topics, batch_atomic_facts, knowledge_source):
        ...
        for j, (atomic_facts, topic) in tqdm(enumerate(zip(batch_atomic_facts, batch_topics)), miniters=25):
            ...
            if atomic_facts is not None and len(batch_atomic_facts) > 0:
                for atom in atomic_facts:
                    ...
                    if self.concat_topic:
                        verification_prompt = self.vllm_verifier.create_messages_single_fact(query=atom,
                                                                                             passages=passages,
                                                                                             topic=topic)
                    else:
                        verification_prompt = self.vllm_verifier.create_messages_single_fact(query=atom,
        ...
        for (start_pos, end_pos) in offsets:
            ...
            for j, (atomic_facts, topic) in enumerate(zip(batch_atomic_facts, batch_topics)):
                ...
                assert len(fact_texts) == len(atomic_facts) == len(decisions)
                if self.debug:
